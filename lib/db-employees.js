...
const getAllManagers = () => {
  // creates a new "thenable" promise
  return new Promise((resolve, reject) => {
    const getQuery = connection.query('SELECT mgr.id, CONCAT(mgr.first_name, " ", mgr.last_name) "Manager" FROM employee_tracker_db.employee mgr where mgr.id in (select emp.manager_id from employee_tracker_db.employee emp)', (err, MgrData) => {
      if (err) {
        consola.error(err);
        reject(err);
        return;
...
const getDepartmentBudget = () => {
  // creates a new "thenable" promise
  return new Promise((resolve, reject) => {
    const getQuery = connection.query('SELECT dept.name, sum(roles.salary) "Budget" FROM role roles INNER JOIN employee_tracker_db.department dept ON roles.department_id = dept.id GROUP BY dept.name', (err, budget) => {
      if (err) {
        consola.error(err);
        reject(err);
        return;
